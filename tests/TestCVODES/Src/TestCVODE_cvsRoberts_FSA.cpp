...
{	std::cout << (tend - tstart) / 1000.0 << "s" << std::endl;}
...
int main(int argc, char *argv[])
{
  ...
  N_Vector absTol_NV;
  absTol_NV = N_VNew_Serial(3);
  NV_Ith_S(absTol_NV, 0) = ATOL1;
  NV_Ith_S(absTol_NV, 1) = ATOL2;
  NV_Ith_S(absTol_NV, 2) = ATOL3;
  CVodeSVtolerances(cvode_mem, RTOL, absTol_NV);
...
//todo - commented cvodes_dense
  ...
  if (sensi) {
	...
	flag = CVodeSensInit1(cvode_mem, NS, sensi_meth, NULL, yS);
